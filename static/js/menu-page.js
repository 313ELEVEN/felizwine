...
function setOrderType(type) {
    const orderType = type === "dine_in" ? "dine_in" : "delivery";
    state.orderType = orderType;

    [els.orderTypeToggle, els.checkoutOrderTypeToggle].forEach((toggle) => {
        toggle?.querySelectorAll("[data-order-type]").forEach((button) => {
            button.classList.toggle("is-active", button.dataset.orderType === orderType);
        });
    });

    const isDelivery = orderType === "delivery";
    els.deliveryFields?.classList.toggle("hidden", !isDelivery);
    els.dineInFields?.classList.toggle("hidden", isDelivery);
}
...
function bindEvents() {
    ...
    [els.orderTypeToggle, els.checkoutOrderTypeToggle].forEach((toggle) => {
        toggle?.addEventListener("click", (event) => {
            ...
            if (button) setOrderType(button.dataset.orderType);
        });
    });
...
function cacheElements() {
    Object.assign(els, {
        ...
        checkoutOrderTypeToggle: document.getElementById("checkout-order-type-toggle"),
