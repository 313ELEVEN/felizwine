...
MENU_SNAPSHOT_FILE = Path("menu_source.json")
...
def ensure_schema_upgrades() -> None:
    """Additively add new columns to existing tables (create_all does not ALTER)."""
    ...
    for name, ddl in ORDER_COLUMN_UPGRADES.items():
...
def migrate_legacy_data() -> None:
    ...
    # menu_source.json is the menu source of truth; seed it first so the legacy
    # imports below act only as a fallback when the snapshot is missing.
    migrated_anything = seed_menu_from_snapshot() or migrated_anything
...
def seed_menu_from_snapshot() -> bool:
    if MenuItem.query.count() > 0 or not MENU_SNAPSHOT_FILE.exists():
        return False

    items = json.loads(MENU_SNAPSHOT_FILE.read_text(encoding="utf-8"))
    for data in items:
        db.session.add(
            MenuItem(
                name=data["name"],
